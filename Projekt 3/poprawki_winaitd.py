...
def explain_MLP(model=None, data_set=None, model_name=None, features=None, classes=None):
    _, ax = plt.subplots(len(classes), 1, figsize=(10, 5))
    attr, _, _ = get_attributions(model=model, input_tensor=data_set.data, target_class=data_set.targets, method="integrated_gradients")
    ...
    for target in range(len(classes)):
        ...
        visualize_attributions(int_grd, input_tensor, model_name=model_name,  method="integrated_gradients_barplot", target_tensor=target_tensor, features=features, target=target, classes=classes, ax=ax[target])
        ax[target].set_ylim([y_min, y_max])
        if target < len(classes) - 1: ax[target].tick_params(axis='x',which='both',bottom=False,top=False,labelbottom=False)
...
if __name__ == "__main__":
    ...
    loading_state_dict()
    explain_MLP(model=model_MLP_iris, data_set=data_MLP_iris, model_name="MLP Iris", features=iris_features, classes=iris_classes)
    explain_MLP(model=model_MLP_wine, data_set=data_MLP_wine, model_name="MLP Wine", features=wine_features, classes=wine_classes)
    explain_MLP(model=model_MLP_breast_cancer, data_set=data_MLP_breast_cancer, model_name="MLP Breast", features=breast_cancer_features, classes=breast_cancer_classes)
