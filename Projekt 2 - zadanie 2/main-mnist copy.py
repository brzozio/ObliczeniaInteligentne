...
if __name__ == "__main__":
    ...
    data_set, input_channels, class_size, data_name, output_channels, kernel_size, image_size = datasets_get.mnist_to_cnn(device,  train)
    #data_set, input_channels, class_size, data_name, output_channels, kernel_size, image_size = datasets_get.cifar10_to_cnn(device,  train)
    ...
    if train is True:
        ...
        for epoch in range(num_epochs):
            data_set.data = data_set.data.view(-1, 1, 28, 28)
        ...
        model = CNN(input_channels=input_channels, output_channels=output_channels, classes=class_size)
        model.load_state_dict(load_model(f'model_{data_name}.pth'))
        model.eval()
        model.double()
        model.to(device)
