...
class CNN(nn.Module):
    def __init__(self, in_side_len: int, in_channels: int, cnv0_out_channels: int,
        ...
        self.conv1_kernel_size: int = convolution_kernel
        ...
        if reduce_to_dim2:
            self.lin0_in_size = self.cnv0_out_channels * (self.in_side_len - self.conv_kernel_size + 1) ** 2
        else:
            conv1_side_len = (self.in_side_len - self.conv_kernel_size + 1) // pooling_kernel
            conv1_side_len = (conv1_side_len - self.conv_kernel_size + 1) // pooling_kernel
            self.conv1_kernel_size = conv1_side_len
            self.lin0_in_size = conv1_side_len * conv1_side_len * self.cnv1_out_channels
